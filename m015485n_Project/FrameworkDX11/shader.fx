...
float DoSpecular(Light lightObject, float3 pixelToEyeVectorNormalised, float3 pixelToLightVectorNormalised, float3 Normal)
{
    ...
    if (lightIntensity > 0.0f)
    {
        ...
        d = saturate(d);
        d = pow(d, 128);
        specular = d; // 32 = specular power Material.SpecularPower
    }
...
{
    LightingResult totalResult = { { 0 }, { 0 } };

	[unroll]
    for (int i = 0; i < MAX_LIGHTS; ++i)
    {
        LightingResult result = { { 0 }, { 0 } };
